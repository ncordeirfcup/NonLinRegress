...
def selected():
    ...
    if Criterion.get()==1:
        estimator1=RandomForestRegressor(random_state=138)
        rn='RF'
    elif Criterion.get()==2:
        estimator1=KNeighborsRegressor()
        rn='KNN'       
    elif Criterion.get()==3:
         estimator1 = AdaBoostRegressor(estimator=DecisionTreeRegressor(random_state=42), random_state=42)
         rn='AB'
        ...
        rn='SVR'        
         ...
         estimator1=GradientBoostingRegressor(verbose=0, random_state=42)
         rn='GB'       
    elif Criterion.get()==6:
         estimator1=MLPRegressor(max_iter=7000, random_state=42)
         rn='MLP'
         ...
         estimator1=XGBRegressor(objective="reg:squarederror",random_state=42)
         rn='XGB'
    elif Criterion.get()==8:
         estimator1=ExtraTreesRegressor(random_state=42)
         rn='ETR'
    elif Criterion.get()==9:
         estimator1=LGBMRegressor(objective='regression',random_state=42)
         rn='LGB' 
    elif Criterion.get()==10:
         estimator1=DecisionTreeRegressor(random_state=42)
         rn='DTR'
    elif Criterion.get()==11:
         estimator1=CatBoostRegressor(random_state=42)
         rn='CBR'  
    else:
        pass
    rn='g_'+rn
...
ttk.Label(settings_frame, text="Correlation cut-off:").grid(row=0, column=0, pady=5, sticky="w")
thirdEntryTabThreer3c1 = ttk.Entry(settings_frame, width=15)
thirdEntryTabThreer3c1.grid(row=0, column=1)

ttk.Label(settings_frame, text="Variance cut-off:").grid(row=1, column=0, pady=5, sticky="w")
fourthEntryTabThreer5c1 = ttk.Entry(settings_frame, width=15)
fourthEntryTabThreer5c1.grid(row=1, column=1)

ttk.Label(settings_frame, text="CV for Grid Search:").grid(row=2, column=0, pady=5, sticky="w")
thirdEntryTabOne = ttk.Entry(settings_frame, width=15)
thirdEntryTabOne.grid(row=2, column=1)

ttk.Label(settings_frame, text="CV for Model Predictability:").grid(row=3, column=0, pady=5, sticky="w")
forthEntryTabOne = ttk.Entry(settings_frame, width=15)
forthEntryTabOne.grid(row=3, column=1)
